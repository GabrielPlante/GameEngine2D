...
namespace ge {
	...
	void RenderableRectangle::render(SDL_Renderer* renderer, const Camera& camera, const Vector2<>& position) const {
		if (camera.isInCamera(Rectangle{ position, w, h })) {
			...
			Vector2<> relativePosition{ camera.absoluteToRelative(position.x, position.y) };
	...
	void RenderableRectangle::renderWithoutCamera(SDL_Renderer* renderer, const Vector2<>& position, SDL_Rect* dstRect) const {
		//Set the color to be rendered the color of the rectangle
		SDL_SetRenderDrawColor(renderer, static_cast<Uint8>(color.red), static_cast<Uint8>(color.green), static_cast<Uint8>(color.blue), static_cast<Uint8>(color.alpha));
